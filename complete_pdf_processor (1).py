...
# --- Table Placeholder Functions (IMPROVED) ---
def get_table_position_info(table, paragraphs):
    """Get position information for a table to determine where to place placeholder."""
    try:
        # Get table's bounding region
        table_regions = table.get('boundingRegions', [])
        if not table_regions:
            return None
            
        table_page = table_regions[0].get('pageNumber', 0)
        table_spans = table.get('spans', [])
        
        if not table_spans:
            return None
            
        # Get the first span to determine table position
        table_start_offset = table_spans[0].get('offset', 0)
        
        return {
            'page': table_page,
            'start_offset': table_start_offset,
            'table_index': None  # Will be set later
        }
    except Exception as e:
        print(f"Error getting table position: {e}")
        return None

def create_table_placeholder(table_idx, table):
    """Create a table placeholder with basic table information."""
    ...
    if cells:
        ...
        placeholder_text = f"[TABLE_{table_idx + 1}]"
        if headers:
            # Clean headers and limit to first 3
            clean_headers = [h for h in headers if h]  # Remove empty headers
            if clean_headers:
                header_text = ', '.join(clean_headers[:3])
                placeholder_text += f" - Headers: {header_text}"
                if len(clean_headers) > 3:
                    placeholder_text += "..."
        
        placeholder_text += f" - Size: {max_row} rows x {max_col} columns"
        
        # Add caption if available
        caption = table.get('caption', {})
        if caption and 'content' in caption:
            caption_text = caption['content'].strip()[:100]  # Limit caption length
            if caption_text:
                placeholder_text += f" - Caption: {caption_text}"
                if len(caption['content']) > 100:
                    placeholder_text += "..."
            
        return placeholder_text
    else:
        return f"[TABLE_{table_idx + 1}] - Empty table"

def find_table_insertion_points(paragraphs, tables):
    """Find where to insert table placeholders in the paragraph flow - IMPROVED VERSION."""
    table_insertions = []
    
    print(f"DEBUG: Finding insertion points for {len(tables)} tables")
    
    for table_idx, table in enumerate(tables):
        position_info = get_table_position_info(table, paragraphs)
        if not position_info:
            print(f"DEBUG: No position info for table {table_idx + 1}")
            continue
            
        position_info['table_index'] = table_idx
        
        # Method 1: Try to find paragraph by page and offset
        best_para_idx = None
        ...
        for para_idx, para in enumerate(paragraphs):
            ...
            para_page = para.get('boundingRegions', [{}])[0].get('pageNumber', 0)
            
            # Check if paragraph is on the same page
            if para_page == position_info['page']:
                # Find paragraph that comes just before the table
                if para_offset <= position_info['start_offset']:
                    distance = position_info['start_offset'] - para_offset
                    if distance < min_distance:
                        min_distance = distance
                        best_para_idx = para_idx
        
        # Method 2: If no good match found, use simpler approach
        if best_para_idx is None:
            print(f"DEBUG: Method 1 failed for table {table_idx + 1}, trying method 2")
            
            # Find paragraphs on the same page as the table
            same_page_paras = []
            for para_idx, para in enumerate(paragraphs):
                para_page = para.get('boundingRegions', [{}])[0].get('pageNumber', 0)
                if para_page == position_info['page']:
                    same_page_paras.append(para_idx)
            
            # Use the middle paragraph on that page as insertion point
            if same_page_paras:
                best_para_idx = same_page_paras[len(same_page_paras) // 2]
                print(f"DEBUG: Using middle paragraph {best_para_idx} on page {position_info['page']}")
        
        # Method 3: Fallback - distribute tables evenly
        if best_para_idx is None:
            print(f"DEBUG: Method 2 failed for table {table_idx + 1}, using fallback")
            # Simple fallback: distribute tables evenly throughout document
            total_paras = len(paragraphs)
            total_tables = len(tables)
            spacing = max(1, total_paras // (total_tables + 1))
            best_para_idx = (table_idx + 1) * spacing
            if best_para_idx >= total_paras:
                best_para_idx = total_paras - 1
        
        if best_para_idx is not None:
            placeholder = create_table_placeholder(table_idx, table)
            table_insertions.append({
                'after_paragraph': best_para_idx,
                'table_index': table_idx,
                'placeholder': placeholder
            })
            print(f"DEBUG: Table {table_idx + 1} will be inserted after paragraph {best_para_idx}")
            print(f"DEBUG: Placeholder: {placeholder}")
    
    # Sort by paragraph index to maintain order
    table_insertions.sort(key=lambda x: x['after_paragraph'])
    print(f"DEBUG: Total insertions planned: {len(table_insertions)}")
...
def extract_content_by_headings_with_tables(paragraphs, all_tables, table_paragraph_indices):
    """Extract content organized by section headings with table placeholders - IMPROVED VERSION."""
    content_structure = {}
    heading_counter = 1
    ...
    print(f"DEBUG: Processing {len(paragraphs)} paragraphs with {len(all_tables)} tables")
    
    # Find table insertion points
    table_insertions = find_table_insertion_points(paragraphs, all_tables)
    table_insertion_map = {item['after_paragraph']: item for item in table_insertions}
    
    print(f"DEBUG: Table insertion map: {list(table_insertion_map.keys())}")
    
    for idx, para in enumerate(paragraphs):
        # Skip if we've already processed this paragraph during Note processing
        ...
        # IMPROVED: Check if we need to insert a table placeholder after this paragraph
        if idx in table_insertion_map:
            table_info = table_insertion_map[idx]
            current_content.append(table_info['placeholder'])
            print(f"DEBUG: Inserted table placeholder after paragraph {idx}: {table_info['placeholder'][:50]}...")
    ...
    print(f"DEBUG: Created {len(content_structure)} content sections")
    
    # Final check: if no placeholders were inserted, try a simple approach
    placeholder_count = sum(1 for content in content_structure.values() 
                           if '[TABLE_' in content.get('combined_content', ''))
    
    if placeholder_count == 0 and all_tables:
        print("DEBUG: No placeholders inserted, trying simple insertion...")
        # Simple fallback: add all table placeholders to the last section
        if content_structure:
            last_key = list(content_structure.keys())[-1]
            for table_idx, table in enumerate(all_tables):
                placeholder = create_table_placeholder(table_idx, table)
                content_structure[last_key]['combined_content'] += f"\n{placeholder}"
                print(f"DEBUG: Added placeholder to last section: {placeholder}")
...
def process_pdf(pdf_name, pdf_folder, output_folder):
    """Process a single PDF file with table placeholders."""
    ...
    print(f"DEBUG: Found {len(all_tables)} raw tables")
    ...
    print(f"DEBUG: After combination: {len(combined_tables)} tables")
    ...
    # Extract content by headings with table placeholders
    content_structure = extract_content_by_headings_with_tables(
        paragraphs, combined_tables, table_paragraph_indices
    )
    ...
    print(f"Saved content with table placeholders to: {json_path}")
...
def test_table_placeholders(pdf_name):
    """Quick test to see if table placeholders are working."""
    pdf_path = os.path.join(pdf_folder, pdf_name)
    ...
    pdf_base = os.path.splitext(pdf_name)[0]
    json_path = os.path.join(output_folder, pdf_base, f"{pdf_base}.json")
    ...
    if os.path.exists(json_path):
        ...
        placeholder_count = 0
        print(f"\n=== CHECKING FOR TABLE PLACEHOLDERS ===")
        for heading_key, heading_data in data.items():
            content = heading_data.get('combined_content', '')
            if '[TABLE_' in content:
                placeholder_count += 1
                print(f"Found placeholder in {heading_key}: {heading_data['heading']}")
                # Show the line with the placeholder
        ...
        if placeholder_count == 0:
            print("No table placeholders found! Check debug output above for issues.")
    else:
...
if __name__ == "__main__":
    ...
    # test_table_placeholders("your_pdf_filename.pdf")