...
def create_table_placeholder(table_idx, table):
    ...
    if cells:
        ...
        if headers:
            placeholder_text += f" - Headers: {', '.join(headers[:3])}" + ("..." if len(headers) > 3 else "")
        ...
        if caption and 'content' in caption:
            ...
            placeholder_text += f" - Caption: {caption_text}" + ("..." if len(caption['content']) > 100 else "")
...
def find_table_insertion_points(paragraphs, tables):
    """Find where to insert table placeholders in the paragraph flow."""
    table_insertions = []
    ...
    for table_idx, table in enumerate(tables):
        ...
        if position_info:
            ...
            # Find the best paragraph position to insert the table placeholder
            best_para_idx = None
            min_distance = float('inf')
            
            for para_idx, para in enumerate(paragraphs):
                # Skip table content paragraphs
                if should_skip_paragraph(para_idx, para, tables, set()):
                    continue
                ...
                if para_spans:
                    ...
                    # Check if paragraph is on the same page and comes before the table
                    if para_page == position_info['page']:
                        distance = abs(para_offset - position_info['start_offset'])
                        if para_offset <= position_info['start_offset'] and distance < min_distance:
...
def extract_content_by_headings_with_tables(paragraphs, all_tables, table_paragraph_indices):
    """Extract content organized by section headings with table placeholders."""
    content_structure = {}
    heading_counter = 1
    ...
    for idx, para in enumerate(paragraphs):
        ...
        # Check if we need to insert a table placeholder after this paragraph
