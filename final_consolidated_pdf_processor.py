...
def get_table_headers(table):
    """Extract column headers from a table (works with both DocumentTable and dict)."""
    headers = []
    
    is_dict = isinstance(table, dict)
    cells = table.get('cells', []) if is_dict else table.cells
    ...
    else: # It's a DocumentTable object
        header_cells = [cell for cell in cells if cell.row_index == 0]
        header_cells.sort(key=lambda x: x.column_index)

    for cell in header_cells:
        content = cell.get('content', '') if is_dict else cell.content
        if content: # Ensure content is not None
             headers.append(content.strip())
    
    return headers

def is_toc_or_appendix_table(table):
    """
    Checks if a table is likely a Table of Contents or an Appendix list.
    """
    ...
    # Keywords that indicate a ToC or Appendix table
    toc_keywords = ['table of contents', 'appendices', 'appendix', 'page']
    
    # Check if any header contains the keywords
    for header in lower_headers:
        for keyword in toc_keywords:
            if keyword in header:
                # A simple "page" header is a strong indicator
                if header == 'page':
                    return True
                # Check for "Table of Contents" or "Appendix"
                if 'table of content' in header or 'appendix' in header:
                    return True
    
    # Also check if the table has exactly two columns and one is 'page'
    if len(lower_headers) == 2 and 'page' in lower_headers:
        return True

    return False

def get_figure_paragraph_indices(result):
    """
    Identifies paragraphs that are part of figures using a two-step verification process.
    Returns a set of paragraph indices to be ignored.
    """
    indices_to_skip = set()

    if not hasattr(result, 'figures') or not result.figures or not hasattr(result, 'paragraphs') or not result.paragraphs:
        return indices_to_skip

    # --- Logic 1: Check for overlapping character spans ---
    figure_spans = []
    for figure in result.figures:
        for span in figure.spans:
            figure_spans.append((span.offset, span.offset + span.length))

    if figure_spans:
        for i, para in enumerate(result.paragraphs):
            if not para.spans:
                continue
            # Using the full range of the paragraph by checking all its spans
            para_start = para.spans[0].offset
            para_end = para.spans[-1].offset + para.spans[-1].length
            
            for fig_start, fig_end in figure_spans:
                # Check for any overlap: max(start1, start2) < min(end1, end2)
                if max(para_start, fig_start) < min(para_end, fig_end):
                    indices_to_skip.add(i)
                    break  # Once an overlap is found, no need to check other figures for this para

    # --- Logic 2: Check for explicit element references ---
    for figure in result.figures:
        if not hasattr(figure, 'elements') or not figure.elements:
            continue
        for element_path in figure.elements:
            if element_path and element_path.startswith('/paragraphs/'):
                try:
                    para_idx = int(element_path.split('/')[-1])
                    indices_to_skip.add(para_idx)
                except (ValueError, IndexError):
                    continue
    
    if indices_to_skip:
        print(f"  - Identified {len(indices_to_skip)} paragraphs as part of figures to be skipped.")
        
    return indices_to_skip

def combine_split_tables(tables):
    """
    Combines tables that are split across pages.
    This is a placeholder and needs a robust implementation based on table structure.
    For now, it returns the tables as-is.
    """
    # This is a complex task. A simple implementation might look for tables
    # on consecutive pages with identical headers.
    # For now, we will not combine and just return the list.
    print("Table combination logic is currently a placeholder.")
    return tables
...
def process_pdf(pdf_name, pdf_folder, output_folder):
    ...
    try:
        ...
        # --- New: Identify paragraphs that are part of figures ---
        figure_paragraph_indices = get_figure_paragraph_indices(result)

        # --- Defer table filtering until output generation ---
        # We use the full table list initially to correctly identify paragraph content within tables.

        # --- New: Dynamically inspect and print available attributes from the result object ---
        print("  - Inspecting Document Intelligence result attributes...")
        ...
        # --- Save the raw Document Intelligence result to JSON ---
        raw_json_path = os.path.join(pdf_output_folder, f"{pdf_base}_raw_DI_result.json")
        try:
            # Re-instating full layout_data structure with as_dict()
            # Filtering of ToC/Appendix tables happens here, just before saving
            layout_data = {
                "api_version": result.api_version,
                "model_id": result.model_id,
                "content": result.content,
                "pages": [page.as_dict() for page in result.pages],
                'paragraphs': [p.as_dict() for p in result.paragraphs] if hasattr(result, 'paragraphs') and result.paragraphs else [],
                'tables': [t.as_dict() for t in result.tables if not is_toc_or_appendix_table(t)] if result.tables else [],
                'figures': [f.as_dict() for f in result.figures] if hasattr(result, 'figures') and result.figures else [],
                'sections': [s.as_dict() for s in result.sections] if hasattr(result, 'sections') and result.sections else [],
                'styles': [s.as_dict() for s in result.styles] if hasattr(result, 'styles') and result.styles else [],
            }

            with open(raw_json_path, "w", encoding="utf-8") as f:
                json.dump(layout_data, f, ensure_ascii=False, indent=4)
            print(f"  - Saved raw Document Intelligence result to: {raw_json_path}")
        except Exception as e:
            print(f"  - Error saving raw Document Intelligence result: {e}")
        ...
        # --- New: Identify all character offsets occupied by ALL tables (including ToC) ---
        # This is crucial to prevent their content from being processed as paragraphs.
        ...
        if result.paragraphs:
            for i, para in enumerate(result.paragraphs):
                # --- New: Skip paragraphs identified as part of a figure ---
                if i in figure_paragraph_indices:
                    continue
        ...
        # 2. Add tables to the list, filtering out ToC/Appendix tables at this stage
        if result.tables:
            print(f"  - Found {len(result.tables)} tables, filtering for ToC/Appendix before adding to unified output...")
            for i, table in enumerate(result.tables):
                if is_toc_or_appendix_table(table):
                    print(f"  - Skipping table on page {table.bounding_regions[0].page_number} (identified as ToC/Appendix).")
                    continue
        ...
        # Filter tables just before generating the Excel file
        final_tables_for_output = []
        if result.tables:
            for table in result.tables:
                if not is_toc_or_appendix_table(table):
                    final_tables_for_output.append(table)

        all_tables_as_dict = []
        if final_tables_for_output:
            for table in final_tables_for_output:
                table_dict = {
        ...
        if all_tables_as_dict:
            ...
            print(f"No tables found in {pdf_name} after filtering.")
